...
SELECT
    (SELECT string_agg(
        ...
        '') 
    FROM generate_series(1, 1 + (random() * 50 + i % 10)::integer)) AS first_name,
    (SELECT string_agg(
        substr(
            '      abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', 
            (random() * 77)::integer + 1, 1
        ...
        '') 
    FROM generate_series(1, 1 + (random() * 50 + i % 10)::integer)) AS last_name,
...
SELECT 
    (1 + random() * 10000)::int AS employee_id,
     (SELECT string_agg(
        substr(
            '      abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', 
            (random() * 77)::integer + 1, 1
        ...
        '') 
    FROM generate_series(1, 1 + (random() * 150 + i % 10)::integer)) AS title,    
    (SELECT string_agg(
        substr(
            '      abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', 
            (random() * 77)::integer + 1, 1
